...
make : $(objects)
	cc -o igc_parser $(objects) -Og -lm -std=c99

%.o: %.c 
	cc -std=c99 -static -Og -c -o $@ $<

.PHONY: clean test format grind

grind:
	for i in {0..11}; do valgrind --tool=memcheck --log-file=$(test_dir)$$i/php.log --leak-check=full --track-origins=yes -v ./igc_parser $(test_dir)$$i/test.igc $(test_dir)$$i; done
