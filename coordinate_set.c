...
void free_coordinate_set_object(coordinate_set_object *intern TSRMLS_DC) {
    ...
    if (intern->coordinate_subset) efree(intern->coordinate_subset);
    if (intern->first_subset) efree(intern->first_subset);
    if (intern->last_subset) efree(intern->last_subset);
    if (intern->coordinate) {
        efree(intern->coordinate);
    }
    efree(intern);
}
...
static zend_function_entry coordinate_set_methods[] = {
        ...
        PHP_ME(coordinate_set, get_date, NULL, ZEND_ACC_PUBLIC)
...
void clone_coordinate_object(coordinate_object *coordinate, coordinate_object *return_intern) {
...
PHP_METHOD (coordinate_set, first) {
    ...
    clone_coordinate_object(intern->first, return_intern);
    RETURN_ZVAL(ret, 0, 0);
}
...
PHP_METHOD (coordinate_set, last) {
    ...
    object_init_ex(ret, coordinate_ce);
    coordinate_object *return_intern = zend_object_store_get_object(ret TSRMLS_CC);
    clone_coordinate_object(intern->last, return_intern);
    RETURN_ZVAL(ret, 0, 0);
}
...
PHP_METHOD (coordinate_set, get) {
    ...
    clone_coordinate_object(coordinate, return_intern);
    RETURN_ZVAL(ret, 0, 0);
}
...
coordinate_subset *create_subset(coordinate_set_object *set, coordinate_object *coordinate) {
    ...
    subset->length = 1;
    subset->first = subset->last = coordinate;
    subset->next = NULL;
...
PHP_METHOD (coordinate_set, get_date) {
    coordinate_set_object *intern = zend_object_store_get_object(getThis() TSRMLS_CC);
    char string[9];
    sprintf(string, "%d/%d/%d", intern->day, intern->month, intern->year);
...
int match_h_record(coordinate_set_object *parser, char *line) {
