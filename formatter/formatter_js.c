...
zend_object_value create_formatter_js_object(zend_class_entry *class_type TSRMLS_DC) {
    ...
    retval.handle = zend_objects_store_put(intern, (zend_objects_store_dtor_t) zend_objects_destroy_object,  (zend_objects_free_object_storage_t)free_formatter_js_object, NULL TSRMLS_CC);
...
PHP_METHOD(formatter_js, output) {
    ...
    RETURN_STRING(json_dumps(json, JSON_PRESERVE_ORDER), 0);
}